...
> [!NOTE]
>
> 阅读dockerfile可知，容器内steam的文件权限与docker主机上的uid为1000的用户相同。
>
> 如有需要，可以修改容器内的PUID指定为其他的用户uid，重新构建镜像。
>
...
工作逻辑：将本地路径挂载到容器内，steamcmd下载时指定目录到挂载的目录，即实际下载到了docker主机本地。该脚本不论安装还是更新都可以使用，且同样适用于其他游戏（需修改应用id）
...
| 变量                  | 说明                                                         | 预设值/默认值                                                |
| --------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `only_print`          | 仅输出模式开关<br />`false`则直接执行`docker run`<br />否则仅标准输出`docker run`代码 | `false`                                                      |
...
> 2. 使用find列出目录中所有文件的路径
> 3. 对文件路径过滤出所有包含 “left4dead2/” 的文件的路径
> 4. 通过神奇的正则表达式裁剪，只留下路径最末尾的"left4dead2/"以及后面的路径，得到用于表示容器内游戏目录的相对路径
> 5. 每个文件单独生成一个 -v 挂载选项，做只读挂载
>
> 就是如此简单粗暴。因此，`addons_dirs`指定的路径，应该是子孙目录中包含left4dead2的目录。如果有加载顺序要求，请按照优先级分别指定路径（后面的目录的文件会覆盖前面目录的文件），确保覆盖顺序不会错误
>
